...
The create-post screen SHALL present lost-pet publication inputs as a simplified visual flow inspired by the provided reference while preserving existing responsive behavior, production validation and backend-compatible field mapping. The screen SHALL present `Características` as an independent optional input immediately after `Nombre`, `Señas particulares` as a second independent optional input immediately after `Características`, and `Detalles adicionales` as the existing general-details input immediately after `Señas particulares`. The `Señas particulares` input SHALL use the independent `particularMarks` attribute, without a required-field marker or label icon.

#### Scenario: User views simplified create-post form

- **GIVEN** a signed-in user opens the create-post screen
- **WHEN** the form is rendered
- **THEN** the primary visible flow presents photo upload, pet name, `Características`, `Señas particulares`, `Detalles adicionales`, manual last-seen location and publish controls without requiring separate visible breed and color inputs
- **AND** `Señas particulares` appears immediately after `Características` and before `Detalles adicionales`

#### Scenario: Particular marks input is independent from existing details

- **GIVEN** the user enters values in `Características`, `Señas particulares` and `Detalles adicionales`
- **WHEN** the user submits the form with the other required inputs and valid media
- **THEN** the app keeps all three values distinct, sending `Características` through `characteristics`, `Señas particulares` through `particularMarks` and `Detalles adicionales` through `features`

#### Scenario: Particular marks input is optional

- **GIVEN** the user leaves `Señas particulares` empty
- **WHEN** the user completes the other required publication inputs and submits the form
- **THEN** the app does not block publication because of the empty `Señas particulares` value
- **AND** the created post carries an empty `particularMarks` attribute

#### Scenario: Particular marks input is persisted locally and remotely

- **GIVEN** the user submits a valid create-post form with a value in `Señas particulares`
- **WHEN** the existing post creation path stores the publication
- **THEN** Room and Firestore persist the value under the independent `particularMarks` attribute
- **AND** reading the post returns the same value without changing `characteristics` or `features`

#### Scenario: Legacy posts remain readable

- **GIVEN** a local Room row or Firestore document created before `particularMarks` existed
- **WHEN** the app reads that post after the change
- **THEN** the post maps successfully with an empty `particularMarks` value and retains all existing fields

#### Scenario: Particular marks label is optional and has no icon

- **GIVEN** the create-post screen is rendered
- **WHEN** the `Señas particulares` input is displayed
- **THEN** the field shows the `Señas particulares` label without a required-field marker
- **AND** the field does not include a label icon or leading icon

#### Scenario: Particular marks label supports both themes
