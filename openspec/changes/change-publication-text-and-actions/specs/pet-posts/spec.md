...
The create-post screen SHALL present lost-pet publication inputs as a simplified visual flow inspired by the provided reference while preserving existing responsive behavior, production validation and backend-compatible field mapping. The screen SHALL use the title `Crea un aviso para ayudar a encontrarla` with the same visual weight as `Toca para agregar una foto`, and its primary submission action SHALL be labeled `Publicar aviso` with a paper-plane icon and matching accessibility description.

#### Scenario: User views simplified create-post form
- **GIVEN** a signed-in user opens the create-post screen
- **WHEN** the form is rendered
- **THEN** the primary visible flow presents the title `Crea un aviso para ayudar a encontrarla` with the same visual weight as `Toca para agregar una foto`, followed by photo upload, pet name, recognition details, manual last-seen location and publish controls without requiring separate visible breed and color inputs
...
- **WHEN** the user submits the form through the `Publicar aviso` action
...
- **THEN** the `Publicar aviso` action remains unavailable or the existing validation error identifies the missing required field without creating a backend post
...
- **THEN** existing top bar, scroll behavior, outer padding rhythm, established field heights and submit-button sizing remain stable with no overlapping or clipped text, and the `Publicar aviso` label and paper-plane icon remain legible
